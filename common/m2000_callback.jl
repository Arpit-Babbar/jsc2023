...
function Base.show(io::IO, ::MIME"text/plain", cb::DiscreteCallback{<:Any, <:StepsizeCallbackM2000})
  @nospecialize cb # reduce precompilation time
...
function initialize!(cb::DiscreteCallback{Condition,Affect!}, u, t, integrator) where {Condition, Affect!<:StepsizeCallbackM2000}
...
@inline function (stepsize_callback::StepsizeCallbackM2000)(integrator)
  ...
  if !integrator.opts.adaptive
    ...
    dt = @trixi_timeit Trixi.timer() "calculate dt" cfl_number * Trixi.max_dt(
      u, t, mesh, Trixi.have_constant_speed(equations), equations, solver, cache)
...
function (cb::DiscreteCallback{Condition,Affect!})(ode::ODEProblem) where {Condition, Affect!<:StepsizeCallbackM2000}
  stepsize_callback = cb.affect!
  @unpack cfl_number = stepsize_callback
  u_ode = ode.u0
  t = first(ode.tspan)
  ...
  dt_ = cfl_number * Trixi.max_dt(u, t, mesh, Trixi.have_constant_speed(equations), equations, solver, cache)
